...
class Benchmark {
 private:
  ...
  void Stop(const std::string_view& name) {
    ...
    std::fprintf(stdout, "%-12s : %11.3f micros/op;%s%s\n",
                 std::string(name).c_str(), (finish - start_) * 1e6 / done_,
 ...
 public:
  ...
  Benchmark()
    ...
    files = Env::Default()->GetChildren(test_dir).value();
  ...
  void Open() {
    ...
    // Open database
    std::string tmp_dir = Env::Default()->GetTestDirectory().value();
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (std::string_view(argv[i]).starts_with("--benchmarks=")) {
      ...
      FLAGS_value_size = n;
               ...
               std::string_view("--no_transaction")) {
  ...
  if (FLAGS_db == nullptr) {
    default_db_path = leveldb::Env::Default()->GetTestDirectory().value();
